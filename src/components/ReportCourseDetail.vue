<template>
  <div class="course-wrap" style="margin-top: 1px;">
    ...
    <ul class="info-list">
      <li>
        <div>私教上课</div>
        <div class="num"><span>{{ personalClassNum }}</span>人</div>
      </li>
      <li style="border: none;">
        <div>团课上课</div>
        <div class="num"><span>{{ leagueClassNum }}</span>人</div>
      </li>
    </ul>
...
//        token: 'BearereyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJsaW5mZW5nIiwiYXVkIjoiODRiYjRkN2M5ZjhmNDViMWJmOWQ5MmM3MGZlZWViOTUiLCJyb2xlIjoiUk9MRV9NRVJDSEFOVCIsImlzcyI6ImNvcmVmYWMiLCJzeXMiOnRydWUsImV4cCI6MTUyMjU3MDMxOH0.vdHpVR0173FFFKEPOHfEUywKOaoxui2j8nTs7UeqGSjj4f0q2Q1drVKfiv6LsKOcpY30QNY9E-7PZKuTwgTtOg',
		...
		methods: {
			...
			getCourseInfo(){
				...
				this.$axios({
					url: this.path + 'index/table/class',
					method: 'get',
					params: {
						startTime: '',
						endStart: '',
					...
					if(res.data.code == 0){
						this.leagueClassNum = res.data.result.leagueClassNum;
						this.personalClassNum = res.data.result.personalClassNum;
						this.classNum = res.data.result.classNum;
					}else {
